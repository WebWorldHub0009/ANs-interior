...
export default function FAQAns() {
  ...
  return (
    <section
      className="relative w-full py-12 px-6 md:px-12 font-[Poppins] overflow-hidden"
    ...
    >
      ...
      <div className="relative text-center mb-16 z-10">
        <h2
          className="text-4xl md:text-5xl mb-4 font-[Great_Vibes]"
          style={{ color: colors.gold, letterSpacing: "1px" }}
        >
          Frequently Asked Questions
        </h2>
        <p className="max-w-2xl mx-auto text-lg md:text-xl text-center" style={{ color: colors.dark }}>
          Explore details about{" "}
          <span style={{ color: colors.gold, fontWeight: 600 }}>Ans Interior</span> and how we bring elegance and comfort into every space.
        </p>
      </div>

      {/* FAQ Container */}
      <div className="relative z-10 flex flex-col md:flex-row max-w-6xl mx-auto shadow-2xl rounded-3xl overflow-hidden border border-[#E0E0E0] bg-white/90 backdrop-blur-lg">
        {/* Sidebar Tabs */}
        <div className="md:w-1/3 bg-[#F8F8F8] p-6 flex flex-col gap-4 border-r border-[#E0E0E0]">
          {Object.entries(faqCategories).map(([category, data], idx) => (
            <motion.button
              ...
              className={`w-full text-left px-5 py-3 rounded-xl flex items-center gap-3 text-base font-medium transition-all duration-300 ${
                activeTab === category
                  ? "bg-[#C5A46D] text-white shadow-xl"
                  : "bg-white text-[#2C2C2C] hover:bg-[#fdf5e6]"
              }`}
            >
              {data.icon}
              <span>{category}</span>
            ...
            >
              {faqCategories[activeTab].faqs.map((faq, index) => (
                <div key={index} className="border-b border-[#E0E0E0] py-4">
                  ...
                  >
                    <span className="text-lg md:text-xl font-medium" style={{ color: colors.dark }}>
                      {faq.question}
                    </span>
                    <motion.span
